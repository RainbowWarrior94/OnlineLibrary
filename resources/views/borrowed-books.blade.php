<x-app-layout>
    ...
    <div class="container">
        <div class="row">
            @if($activeBorrowedBooks && count($activeBorrowedBooks) > 0)
                @foreach($activeBorrowedBooks as $book)
                    <div class="py-2">
                        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                                <div class="p-6 text-gray-900 dark:text-gray-100">
                                    ...
                                    <p><strong>Borrowed at: </strong>{{ Carbon\Carbon::parse($book->pivot->borrowed_at)->format('d.m.Y') }}</p> 
                                    ...
                                    <div style="text-align: right;">
                                        <form id="returnForm{{ $book->id }}" class="inline-form">
                                            @csrf
                                            <input type="hidden" name="user_id" value="{{ Auth::id() }}">
                                            <button type="submit" class="borrow-button" data-book-id="{{ $book->pivot->id }}">Return</button>
                                        </form>
                                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var returnButtons = document.querySelectorAll('.borrow-button');

            returnButtons.forEach(function(button) {
                button.addEventListener('click', function(event) {
                    event.preventDefault();
                    var bookId = button.getAttribute('data-book-id');
                    window.location.href = "{{ route('pay', ':id') }}".replace(':id', bookId);
                });
            });
        });
    </script>
